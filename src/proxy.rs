...
async fn run_client(upper_shutdown: oneshot::Receiver<()>, context: TrojanContext) -> Result<()> {
    // let (shutdown_tx, _) = broadcast::channel(1);
...
async fn run_quic_server(
    ...
    while let Some(conn) = incoming.next().await {
        ...
        let resolver = context.dns_resolve_tx.clone();
        ...
        tokio::spawn(async move {
            handle_quic_connection(bi_streams, shutdown_rx, hash_copy, fallback_port, resolver)
                .await
